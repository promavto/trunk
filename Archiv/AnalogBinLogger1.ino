...
//  info_ADC((metadata_t*) &block[0]);
  ...
  while (1) {
	...
	//*********************************************************************
	if (myTouch.dataAvailable())
//	if (Serial.available())   // Окончание измерения
	{
	  // Stop ISR calls.
	  adcStop();
	  if (isrBuf != 0 && isrBuf->count >= PIN_COUNT) 
  ...
  if (!binFile.rename(sd.vwd(), binName))
   {
	 error("Can't rename file");
	  ...
	  //myGLCD.print("File: ",LEFT, 25);//"Сброс"
	  //myGLCD.setColor(VGA_YELLOW);
	  //myGLCD.print(binName,100 , 25);// Печать номера входов
	  //myGLCD.setColor(255, 255, 255);
	  ////myGLCD.print("Max block write :",LEFT, 45);//"Сброс"
	  //myGLCD.setColor(VGA_YELLOW);
	  //myGLCD.printNumI(maxLatency, 230, 45);// 
	  //myGLCD.setColor(255, 255, 255);
	  //myGLCD.print("Record time sec: ",LEFT, 65);//"Сброс"
	  //myGLCD.setColor(VGA_YELLOW);
	  //myGLCD.printNumI(0.001*(t1 - t0), 230, 65, 3);// 
	  //myGLCD.setColor(255, 255, 255);
	  //myGLCD.print("Sample Rate: ",LEFT, 85);//"Сброс"
	  //myGLCD.setColor(VGA_YELLOW);
	  //myGLCD.printNumI(sampleRate, 230, 85);// 
	  //myGLCD.setColor(255, 255, 255);
	  //myGLCD.print("Sample interv: ",LEFT, 105);//"Сброс"
	  //myGLCD.setColor(VGA_YELLOW);
...
void drav_Menu1()
{
	...
	myGLCD.print("Record ADC data", CENTER, 30);//"Сброс"
	myGLCD.print("Convert to CSV", CENTER, 80);//"Сброс"
	myGLCD.print("Data to Serial", CENTER, 130);//"Сброс"
	myGLCD.print("Error details", CENTER, 180);//"Сброс"
}
			...
			if ((x>=30) && (x<=290))  // Upper row
			  {
				if ((y>=20) && (y<=60))  // Button: 1
				  {
				//	myGLCD.print("1", CENTER, 220);//"Сброс
					myGLCD.clrScr();
					logData();
					//logData_Monitor();
				//	info_measure();
				...
				if ((y>=70) && (y<=110))  // Button: 1
				  {
					myGLCD.print("2", CENTER, 220);//"Сброс
					binaryToCsv();
					return;
				  }
				if ((y>=120) && (y<=160))  // Button: 1
				  {
					myGLCD.print("3", CENTER, 220);//"Сброс
					dumpData();
					return;
				  }
				if ((y>=170) && (y<=220))  // Button: 1
				  {
					myGLCD.print("4", CENTER, 220);//"Сброс
					checkOverrun();
					return;
				  }
