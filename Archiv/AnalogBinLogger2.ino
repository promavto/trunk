...
//------------------------------------------------------------------------------
// Buffer definitions.Определения буфера.
//
...
ISR(ADC_vect) 
{
  ...
  // Check for buffer needed. Необходимо проверить буффер
  if (isrBufNeeded) 
  {   
  ...
  // Store ADC data. Запись данных АЦП.
...
void logData1()
{
	...
	// Create new file.
	//  myGLCD.print("Creating new file",LEFT, 155);//"Сброс"
	Serial.println(F("Creating new file"));
	binFile.close();
	if (!binFile.createContiguous(sd.vwd(),
	TMP_FILE_NAME, 512 * FILE_BLOCK_COUNT))
	...
	// Flash erase all data in the file.
	//  myGLCD.print("Erasing all data",LEFT, 175);//"Сброс"
	Serial.println(F("Erasing all data"));
	uint32_t bgnErase = bgnBlock;
	uint32_t endErase;
	while (bgnErase < endBlock) 
...
void start_ADC()
{
	...
	//  Draw_menu_ADC1();
	  char c;
		...
		while (Serial.read() >= 0) {} // Очистить буффер приема
	  //Serial.println();
	  ...
	  // Read any extra Serial data.  Получить всю информацию из буффера
	  do {
		delay(10);
	  } while (Serial.read() >= 0);
